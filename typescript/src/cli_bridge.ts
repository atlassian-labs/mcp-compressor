...
import { formatToolHelp, formatTopLevelHelp, parseArgvToToolInput, toolNameToSubcommand } from './cli_tools.js';
...
export class CliBridge {
  ...
  private readonly runtime: CompressorRuntime;
  private server: http.Server | null = null;
  private toolBySubcommand = new Map<string, Tool>();

  constructor(runtime: CompressorRuntime, cliName: string) {
  ...
  private async handleRequest(request: http.IncomingMessage, response: http.ServerResponse): Promise<void> {
    ...
    if (request.method === 'GET' && toolHelpMatch) {
      ...
      if (!tool) {
        this.send(response, 404, `Unknown subcommand: ${decodeURIComponent(toolHelpMatch[1]!)}\n\n${formatTopLevelHelp(this.cliName, this.cachedTools ?? [])}`);
        return;
      }
    ...
    if (request.method === 'POST' && toolInvokeMatch) {
      ...
      if (!tool) {
        this.send(response, 404, `Unknown subcommand: ${subcommand}\n\n${formatTopLevelHelp(this.cliName, this.cachedTools ?? [])}`);
        return;
      }
  ...
  private async invokeToolFromArgv(tool: Tool, argv: string[]): Promise<{ statusCode: number; body: string }> {
    try {
